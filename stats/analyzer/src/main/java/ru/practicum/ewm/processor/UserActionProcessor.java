...
public class UserActionProcessor {
    ...
    public void start() {
        ...
        try {
            ...
            log.error("{}: Ошибка во время обработки userAction", ru.practicum.ewm.processor.UserActionProcessor.class.getSimpleName(), e);
        } finally {
            try {
                userActionConsumer.commitSync();
            } finally {
                log.info("{}: Закрываем консьюмер", ru.practicum.ewm.processor.UserActionProcessor.class.getSimpleName());
